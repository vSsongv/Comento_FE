...
const member = {
  signup: asyncHandler(async (req, res, next) => {
    ...
    if (!regEmail.test(email)) return next(new errorResponse(detailResponse.EMAIL_FORM, 400));

    if (!password) return next(new errorResponse(detailResponse.EMPTY_PASSWORD, 400));

    if (password.length < 8) return next(new errorResponse(detailResponse.PASSWORD_LENGTH_ERROR, 400));

    if (!regPassword.test(password)) return next(new errorResponse(detailResponse.PASSWORD_FORM, 400)); //비밀번호는 영문, 숫자, 특수문자 포함 8~16자

    if (!phone) return next(new errorResponse(detailResponse.EMPTY_PHONE, 400));

    if (!regPhoneNum.test(phone)) return next(new errorResponse(detailResponse.PHONE_FORM, 400));

    if (!nickname) return next(new errorResponse(detailResponse.EMPTY_NICKNAME, 400));

    if (nickname.length > 10 || nickname.length < 1)
      return next(new errorResponse(detailResponse.NICKNAME_LENGTH_ERROR, 400));

    if (!regNickname.test(nickname)) return next(new errorResponse(detailResponse.NICKNAME_FORM), 400);

    const checkEmail = await userService.checkEmail(email);
    if (checkEmail) return next(new errorResponse(basicResponse(detailResponse.DUP_EMAIL), 400));

    const checkNickname = await userService.checkNickname(nickname);
    if (checkNickname) {
      return next(new errorResponse(basicResponse(detailResponse.DUP_NICKNAME), 400));
    }

    const checkPhone = await userService.checkPhone(phone);
    if (checkPhone) return next(new errorResponse(basicResponse(detailResponse.DUP_PHONE), 400));
  ...
  checkId: asyncHandler(async (req, res, next) => {
    ...
    const checkEmail = await userService.checkEmail(email);
    if (!checkEmail) return res.send(basicResponse(detailResponse.AVAILABLE_EMAIL));
  ...
  checkPhone: asyncHandler(async (req, res, next) => {
    ...
    if (!checkPhone) return res.send(basicResponse(detailResponse.AVAILABLE_PHONE));
  ...
  checkNickname: asyncHandler(async (req, res, next) => {
    ...
    if (!checkNickname) return res.send(basicResponse(detailResponse.AVAILABLE_NICKNAME));
  ...
  findPassword: asyncHandler(async (req, res, next) => {
    ...
    if (!isUser) return next(new errorResponse(detailResponse.NOT_EXIST_EMAIL, 400));
  ...
  updateMentoRole: asyncHandler(async function (req, res, next) {
    ...
    if (!regNumber.test(userIdx))
      return next(new errorResponse(basicResponse(detailResponse.TOKEN_VERFICATION_FAIL), 400));
  ...
  updateNickname: asyncHandler(async function (req, res, next) {
    ...
    if (!regNumber.test(userIdx))
      return next(new errorResponse(basicResponse(detailResponse.TOKEN_VERFICATION_FAIL), 400));
    if (!nickname) return next(new errorResponse(basicResponse(detailResponse.EMPTY_NICKNAME), 400));
    await userService.updateUserNickname(nickname, userIdx);
    return res.send(resultResponse(detailResponse.PATCH_NICKNAME_SUCCESS, nickname));
  }),
  updatePassword: asyncHandler(async function (req, res, next) {
    ...
    if (prevPassword && password == 0)
      return next(new errorResponse(basicResponse(detailResponse.EMPTY_PASSWORD), 400));
    ...
    if (!isEqualPw) return next(new errorResponse(basicResponse(detailResponse.NOT_MATCH_PASSWORD), 400));
  ...
  updateProfile: asyncHandler(async function (req, res, next) {
    ...
    try {
      const userIdx = req.user.userid;
      if (!profile) next(new errorResponse(basicResponse(detailResponse.EMPTY_PROFILE), 400));
      ...
      return res.send(resultResponse(detailResponse.PATCH_PROFILE_SUCCESS, profile));
    } catch (error) {
      const params = {
        Bucket: secret.bucket,
        Key: profile,
  ...
  authRequestEmail: asyncHandler(async function (req, res, next) {
    ...
    if (!regNumber.test(userIdx))
      return next(new errorResponse(basicResponse(detailResponse.TOKEN_VERFICATION_FAIL), 400));
    if (content.length > 200) return next(new errorResponse(basicResponse(detailResponse.CONTENT_LENGTH_OVER), 400));
    const userInfo = await userService.getUserInfo(userIdx);

...
function SignupModal({ onLogin }: SignUpModalProps) {
    ...
    <RootContainer>
      <LoginModal width={"784px"} height={"495px"}>
        ...
        <div onClick={() => {
            ...
            if ('emailError' in response)
                setEmailError(String(response.emailError));
            if ('pwError' in response)
                setPwError(String(response.pwError));
            }}>

...
interface SignUpModalProps {
  onRegister: (email: string, pw: string, nickname: string, phoneNumber: string) => SignUpResultData;
}
...
const getPwConfirmError = (pw: string, pwC: string) => (pw == pwC ? '' : '비밀번호가 일치하지 않습니다.');
const getNickError = (s: string) => (s ? '' : '닉네임을 입력하세요.');
      ...
      <LoginModal width='100%' height='1027px'>
        ...
        <div
          onClick={() => {
            ...
            if (emailError || pwError || pwConfirmError || nickError || phoneError) allFine = false;
